  ...
  return <details className="utility-panel">
    ...
    {conversationId && <>
      ...
      {branches?.selected === conversationId && <>
        ...
        <ol>{branches.nodes.map(node => <li key={node.id}>{node.role} · {node.status} · <code>{node.upstreamNodeId ?? "No upstream node"}</code>
          {node.role === "assistant" && node.upstreamNodeId && <button disabled={disabled} onClick={() => void branch(node.id)}>Continue from this assistant</button>}
        </li>)}</ol>
      </>}
